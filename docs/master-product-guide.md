# BMV Staff Portal — Master Product Guide v1.0
...
2. **Add the Skills.** Create `.agents/skills/` in the repo root with one subfolder per agent in §8 (`agent_orchestrator/`, `agent_architect/`, `agent_designer/`, `agent_compliance/`, `agent_developer/`, `agent_qa_devops/`, `agent_docs/`), each with a `SKILL.md` containing that agent's Role + System Instruction + Execution Rule verbatim from §8. This is what makes the instructions load automatically instead of being retyped into chat every session. Also commit this guide itself into the repo as `docs/master-product-guide.md`, and start every `SKILL.md` with one line — `Full spec: docs/master-product-guide.md` — so that any §N or REQ-XX-NN citation inside the instructions is something the agent can actually open and read, not a dangling reference to a document it was never given.
